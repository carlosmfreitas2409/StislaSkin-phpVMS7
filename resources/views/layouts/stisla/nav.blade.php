...
<div class="main-sidebar sidebar-style-2">
    <aside id="sidebar-wrapper">
        <div class="sidebar-brand">
            <a href="{{ route('frontend.dashboard.index') }}">{{ config('app.name') }}</a>
        </div>

        <div class="sidebar-brand sidebar-brand-sm">
            @if(Auth::check())
                @if (Auth::user()->avatar == null)
                    <a href="{{ route('frontend.dashboard.index') }}"><img src="{{ Auth::user()->gravatar(38) }}" class="rounded-circle" style="width: calc(100% - 31px);"></a>
                @else
                    <a href="{{ route('frontend.dashboard.index') }}"><img src="{{ Auth::user()->avatar->url }}" class="rounded-circle" style="width: calc(100% - 31px);"></a>
                @endif
            @else
                <a href="{{ route('frontend.dashboard.index') }}">VMS</a>
            @endif
